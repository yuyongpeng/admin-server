...
export class SysTicketController {
  ...
  @ApiOperation({ summary: "查询 ticket 信息 " })
  @ApiResponse({ type: Result<ticket> })
  @RequirePermission("system:ticket:query")
  @Get("/getMulti/:ids") // ids=1,2,3,4  经过 ParseIntArrayPipe 会转换为 [1,2,3,4]
  async getTickets(@Param("ids", ParseIntArrayPipe) ids: number[]): Promise<Result<ticket[]>> {
    return Result.ok(await this.ticketService.selectTicketByIds(ids));
  }
